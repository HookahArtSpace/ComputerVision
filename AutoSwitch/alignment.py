...
def alignPair(f1, f2, matches, m, nRANSAC, RANSACthresh):
    ...
    for i in range(nRANSAC):
        H = np.eye(3)
        if m == eTranslate:
            rm = random.choice(matches)
            (a_x, a_y) = f1[rm.queryIdx].pt
            (b_x, b_y) = f2[rm.trainIdx].pt
            H[:2,2] = np.array((b_x - a_x, b_y - a_y))
        elif m == eHomography:
            rm = random.choices(matches, k = 4)
            H = computeHomography(f1, f2, rm)
        else:
            raise Exception('Error in Motion Model Type')
...
def getInliers(f1, f2, matches, M, RANSACthresh):
    ...
    for i in range(len(matches)):
        ...
        f1_queryPt = np.array([a_x, a_y, 1])
        f1_trans = M @ (f1_queryPt.T)

        f1_norm = f1_trans[:2]/f1_trans[2]

        f2_queryPt = np.array(f2[matches[i].trainIdx].pt)

        l2_dis = np.linalg.norm(f1_norm - f2_queryPt)
        if l2_dis <= RANSACthresh:
