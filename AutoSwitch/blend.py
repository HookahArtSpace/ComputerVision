...
def imageBoundingBox(img, M):
    ...
    corners_new =[M @ corners[i].T for i in range(len(corners))]
    corners_norm = np.array([corner/corner[2] for corner in corners_new])
    minX = np.min(corners_norm[:,0])
    minY = np.min(corners_norm[:,1])
    maxX = np.max(corners_norm[:,0])
    maxY = np.max(corners_norm[:,1])
    #TODO-BLOCK-END
...
def accumulateBlend(img, acc, M, blendWidth):
    ...
    acc_h, acc_w = acc.shape[0], acc.shape[1]
    img_h, img_w = img.shape[0]. img.shape[1]
    weight_l = np.linspace(0, 1, blendWidth)
    weight_r = np.linspace(1, 0, blendWidth)
    weights = np.concatenate((weight_l, np.ones(maxX - minX - 2 * blendWidth),weight_r))

    img_new = np.ones((img_h, img_w, 4))
    img_new[:,:,:3] = img[:,:,:3]

    rgb = cv2.warpPerspective(img_new, M, (acc_w, acc_h), cv2.INTER_LINEAR)

    for i in range(minX, maxX):
        for j in range(minY, maxY):
            rgb[j,i,:3] = rgb[j,i,:3] * weights[i - minX]
            rgb[j,i,3] = weights[i - minX]

            if rgb[j,i,:3] == np.zeros(3):
                rgb[j,i,3] = 0
            acc[j,i] = rgb[j,i]
...
def getAccSize(ipv):
    ...
    for i in ipv:
        ...
        #TODO-BLOCK-BEGIN
        minX_new, minY_new, maxX_new, maxY_new = imageBoundingBox(img, M)
        minX = min(minX, minX_new)
        minY = min(minY, minY_new)
        maxX = max(maxX, maxX_new)
        maxY = max(maxY, maxY_new)
        #TODO-BLOCK-END
        # END TODO
