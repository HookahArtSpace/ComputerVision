...
class HarrisKeypointDetector(KeypointDetector):
    ...
    def computeHarrisValues(self, srcImage):
        ...
        ixy = dx * dy
        iyy = dy * dy
        gixx = ndimage.gaussian_filter(ixx, 0.5 )
        gixy = ndimage.gaussian_filter(ixy, 0.5 )
        giyy = ndimage.gaussian_filter(iyy, 0.5)
        harrisImage = gixx * giyy - gixy ** 2 - 0.1 * (gixx + giyy) ** 2
    ...
    def computeLocalMaxima(self, harrisImage):
        ...
        # TODO-BLOCK-BEGIN
        destImage = np.zeros_like(harrisImage, np.bool)
        local_maxima = ndimage.maximum_filter(harrisImage,size = (7,7))
...
class SimpleFeatureDescriptor(FeatureDescriptor):
    ...
    def describeFeatures(self, image, keypoints):
        ...
        padImage = np.zeros((grayImage.shape[0]+4,grayImage.shape[1]+4))
        padImage[2:grayImage.shape[0]+2,2:grayImage.shape[1]+2] = grayImage
...
class MOPSFeatureDescriptor(FeatureDescriptor):
    ...
    def describeFeatures(self, image, keypoints):
        ...
        for i, f in enumerate(keypoints):
            ...
            rotMx = transformations.get_rot_mx(0,0,angle)
            scaMx = transformations.get_scale_mx(0.2,0.2,0)
            transMx1 = transformations.get_trans_mx(np.array((-x,-y,0)))
            transMx2 = transformations.get_trans_mx(np.array((windowSize/2,windowSize/2,0)))
            transMx = np.dot(transMx2, np.dot(scaMx, np.dot(rotMx, transMx1)))[:2,(0,1,3)]
            # TODO-BLOCK-END
            ...
            # TODO-BLOCK-BEGIN
            normImage = destImage - np.mean(destImage)
            var = np.var(destImage)
            if var < 1e-10:
                desc[i,:] = np.zeros((windowSize * windowSize))
            else:
                desc[i,:] = (normImage / np.sqrt(var)).reshape((windowSize * windowSize))
...
class SSDFeatureMatcher(FeatureMatcher):
    def matchFeatures(self, desc1, desc2):
        ...
        for i, dis in enumerate(distances):
            minIdx = np.argmin(dis)
            match = cv2.DMatch()
            match.queryIdx = i
            match.trainIdx = minIdx
            match.distance = dis[minIdx]
...
class RatioFeatureMatcher(FeatureMatcher):
    def matchFeatures(self, desc1, desc2):
        ...
        distances = spatial.distance.cdist(desc1,desc2,metric='euclidean')
        for i, dis in enumerate(distances):
            minIdx, secIdx = np.argsort(dis)[:2]
            match = cv2.DMatch()
            match.queryIdx = i
            match.trainIdx = minIdx
            match.distance = dis[minIdx]/dis[secIdx]
            matches.append(match)
        # TODO-BLOCK-END
